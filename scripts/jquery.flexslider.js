    ...
    slider.flexAnimate = function(target, pause, override, withSync, fromNav) {
      if (!slider.animating && (slider.canAdvance(target) || override) && slider.is(":visible")) {
        ...
        if (!fade) {
          ...
          var adjust = $("li#street").css("width");
          adjust = parseInt(adjust, 10);
          var mycurrentwidth = $(window).width();
          if ((mycurrentwidth <= 650) && (initial == 1)) {
              dimension = (vertical) ? slider.slides.filter(':first').height() : adjust, margin, slideString, calcNext;
          } 
    ...
    slider.setup = function(type) {
      ...
      if (!fade) {
        ...
        if (vertical && !carousel) {
          ...
          var mywidthchange = $(window).width();
          if (mywidthchange <= 480) {
              slider.setProps(mywidthchange, "init");
          } else {
          ...
          setTimeout(function(){
            slider.doMath();
            if (mywidthchange <= 480) {
                slider.newSlides.css({"width": mywidthchange, "float": "left", "display": "block"});
            } else {
